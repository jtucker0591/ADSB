...
**Running more than one board:** each physical board needs its own `secrets.h` at the time you first flash it over USB, since that's what sets its WiFi and home location. After that first boot, the board saves its own identity into its own flash (NVS) automatically — it's no longer tied to whatever's compiled into `secrets.h`, and won't be affected by future shared OTA updates (see below). You only touch `secrets.h` again when building for a brand-new board (or re-flashing an existing one from scratch).

Each board's real values are kept in **`../board-configs/`**, a plain folder that sits next to this repo (not inside its git working tree — see below for why), one file per board: `cp ../board-configs/<board>.secrets.h src/secrets.h` before building for that board. See `board-configs/README.md` for the current list.
...
**On keeping per-board secrets out of git, after an actual incident:** on 2026-09-21, backup copies of `secrets.h` for four boards (real WiFi passwords for four different networks — kept in `src/` at the time, as `secrets.h.<board>.bak`, for juggling more than one board's identity on the same machine) got committed and pushed to this public repo by accident. That's fixed (files removed, git history rewritten, passwords rotated), and there are now several layers so it can't happen the same way again:

- **Structural fix:** per-board secrets no longer live inside the repo at all — see `../board-configs/` above. A file that was never in the working tree can't be swept into a commit regardless of what any filter below misses.
- `.gitignore` still covers `src/secrets.h.*` and `*.bak` generally (not just the exact `src/secrets.h` path), in case a board file ever ends up in `src/` temporarily.
